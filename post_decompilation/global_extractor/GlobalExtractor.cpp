...
public:
  ...
  bool VisitFunctionDecl(FunctionDecl *func) {
    if (func->isMain()) {
      ...
      string main_sign = getEnvVar("MAIN_SIGN_TXT");
      appendLineToFile(main_sign, func_text);
    }
    return true;
  }
  ...
  bool VisitVarDecl(clang::VarDecl *NamedDecl) {
    if (NamedDecl->hasGlobalStorage() && !(NamedDecl->isStaticLocal())) {
      ...
      llvm::outs() << decl_str << "\n";
      string global_info = getEnvVar("GLOBAL_INFO_TXT");
      appendLineToFile(global_info, decl_str + "\n");
    }
  ...
  bool VisitRecordDecl(clang::RecordDecl *rd) {
    ...
    string record_info = getEnvVar("RECORD_INFO_TXT");
    appendLineToFile(record_info,
    ...
    for (RecordDecl::field_iterator j_se = rd->field_begin(),
      ...
      appendLineToFile(record_info, decl_str + "\n");
    }
    return true;
  }
};
...
int main(int argc, const char **argv) {
  ...
  CommonOptionsParser& op = parser.get();
  ...
  return Tool.run(newFrontendActionFactory<GlobalExtractorFrontendAction>().get());
}
